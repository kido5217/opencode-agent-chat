...
export function renderMessageLine(m: Message): string {
  const flag = m.kind === "question" ? "? " : m.kind === "blocker" ? "! " : "";
  const to = m.to_name === null ? "" : ` → ${sanitize(m.to_name)}`;
  return `${flag}[${m.id}] ${sanitize(m.sender_name)} · ${m.kind}${to}: ${excerpt(m.body)}`;
}

export function renderMessages(messages: Message[]): string {
  return messages.map(renderMessageLine).join("\n");
}
